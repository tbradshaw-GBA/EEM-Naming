...
originalList <- read_excel("N:\\TBradshaw\\R\\ECM Naming\\ECM Master List Original.xlsx")

# Define output path -- replace file path with your file path
outputPath <- "N:\\TBradshaw\\R\\ECM Naming\\ECM Total Test EEM Naming.xlsx"
...
adjust <- c("adjust","adjustments","reduce","set","turn","minimize","increase","lower","optimize","reset",
            "supply","avoid","correct","vary","refine","modification","modifications","modify","revise","comissioning")
...
actionLabels <- list(
  ...
  Remove = remove[1], Repair = repair[1], Weatherize = weatherize[1]
)
...
actionCategories <- list(
  ...
  Remove = tolower(remove), Repair = tolower(repair), Weatherize =tolower(weatherize)
)

# Define original word vectors for system categories
lighting <- c("Lighting","light","lumen","occupancy","florescent","lamp","lamps")
airDistribution <- c("Air Distribution","ventilation","airflow","airflows","ahu","duct","hvac","exhaust","vfd",
                     "ahu's","ahus","VAV","variable air volume","VS","fan","fans","supply","air handling unit","air handling units",
                     "economizer","variable air volume")
coolingSystem <- c("Cooling System","chiller","chw","cw","chilled","cooling","cool","thermostat")
heatingSystem <- c("Heating System","heat","heater","heating","boiler","hw","thermostat","reheat","hot water","steam")
controlSystem <- c("Controls", "BAS","automation","user based","user based controls","setpoint","control",
                   "controls","sensor","sensors","schedule")
waterSystem <- c("Water System","sewer","condenser","pump")
...
systemLabels <- list(
  ...
  WaterSystem = waterSystem[1], EnergySupply = energySupply[1], Envelope = Envelope[1]
)
...
for (i in 1:nrow(originalList)) {
  ...
  desc_clean <- gsub("(?<=^|\\s)S(?=\\s|$)", "AHU", desc_clean, perl = TRUE)   # replaces S-### with AHU
  desc_clean <- gsub("(?<=^|\\s)VS(?=\\s|$)", "AHU", desc_clean, perl = TRUE)   # replaces S-### with AHU
  desc_clean <- gsub("(?<=^|\\s)CH(?=\\s|$)", "Chiller", desc_clean, perl = TRUE)   # Ch with Chiller
  ...
  locationList <- c("area", "lab", "laboratory", "office", "elevator", "lobby", "vending machine", "ER", "cath", "stairwell",
                    "stair","halogen","parking garage","kitchen","cafeteria","surgery","hall","ballroom","garage")
...
write.xlsx(result, outputPath, rowNames = FALSE)
cat("Tagged rows saved to:", outputPath, "\n")
